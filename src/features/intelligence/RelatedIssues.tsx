...
export default function RelatedIssues({ relations }: RelatedIssuesProps) {
  ...
  const duplicates = relations.filter(r => r.relationType === 'DUPLICATE');
  const relateds = relations.filter(r => r.relationType === 'RELATED');
            ...
            duplicates.map((item, idx) => (
              <div key={idx} className="nexus-card nexus-card-ruby" style={{
                ...
                <div style={{ marginTop: '8px', fontSize: '11px', color: 'var(--text-secondary)' }}>
                  <strong>Causal Explanation:</strong> {item.connectionExplanation}
                </div>

                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginTop: '8px' }}>
                  {item.sharedSignals.map((sig, sIdx) => (
